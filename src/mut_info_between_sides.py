...
def momentum_measured(Gamma,un_set,meas_set):
    na = un_set.shape[0]//2
    nb = meas_set.shape[0]//2
...
def tfd(n_tube,L,Lh,mu_s,t):
    ...
    Gamma_TFD = momentum_measured(Gamma_q,un_set,meas_set)
...
"""
N = 64
tube_lengths = np.arange(0,120)

mi_left_right = []
for n in range(len(tube_lengths)):
    Gamma_TFD = tfd(n_tube = tube_lengths[n],L=7,Lh=5,mu_s=1,t=10)
    mi_left_right.append(mutual_information(Gamma_TFD,list(range(0,N)),list(range(N,2*N))))
    print(mi_left_right[-1])

plt.plot(tube_lengths,mi_left_right)
plt.xlabel("tube length")
plt.ylabel("mutual information between sides")
plt.show()
"""


N = 64
quench_times = np.arange(1,50)

mi_left_right = []
for t in range(len(quench_times)):
    Gamma_TFD = tfd(n_tube = 1,L=7,Lh=5,mu_s=1,t=quench_times[t])
    mi_left_right.append(mutual_information(Gamma_TFD,list(range(0,N)),list(range(N,2*N))))
    print(mi_left_right[-1])

plt.plot(quench_times,mi_left_right)
