...
def teleportation_protocol(s,theta,insert_idx,wormhole,n_one_side,n_tube,t_couple,t0,cutoff,H_coupling):
    q = insert_idx
    if wormhole == False:
        N = 2*n_one_side
        k = 5
...
def Vout_ideal(Vin,r):
    Vout = Vin + .5*np.exp(-2*r)*np.eye(Vin.shape[0])
    return Vout
...
def fidelity_channel(
    ...
    for s, theta in input_ensemble:
        ...
        Vouts.append(extract_subsystem_covariance(Gamma_final,[insert_idx+N]))
...
def fidelity_ideal(input_ensemble,r,noise):    

    Vins = []
    Vouts = []

    for s, theta in input_ensemble:
        Vin = make_input_covariance(s,theta)
        Vins.append(Vin)
        Vouts.append(Vout_ideal(Vin,r))
...
channel_fidelities_flip=[]
channel_fidelities_symp =[]
...
for s in range(len(sizes)):
    H_coupling_mat = H_coupling(sizes[s])
    Fs, Ff = fidelity_channel(
    ...
    t0=times[s],
    cutoff=400,
    H_coupling=H_coupling_mat)
    channel_fidelities_symp.append(Fs)
    channel_fidelities_flip.append(Ff)


plt.rc('font', size=14)
#plt.plot(sizes,channel_fidelities_symp,label="symplectic")
plt.plot(sizes,channel_fidelities_flip,color="black",linewidth=2,label="many body teleportation")
plt.axhline(.711,color="blue",linestyle="dashed",linewidth=2,label="standard teleportation fidelity = .711")
plt.xlabel("sites on one side")
plt.ylabel("fidelity")
plt.title("Fidelity vs System Size")
plt.legend()
plt.show()



rlist = np.linspace(.05,4,40)
noise = 0 *np.eye(2)
fid_ideal = []
for r in rlist:
    fid_ideal.append(fidelity_ideal(input_ensemble,r,noise))
   



plt.rc('font', size=26)
plt.plot(rlist,fid_ideal,color="k",linewidth=4)
plt.yticks([0.6, 0.8,1])
plt.axvline(.328,color="blue",linestyle="dashed",linewidth=4,label = "r=.328")
...
noise_list = np.linspace(0,10,50)
r=1

for n in noise_list:
    fid_noisy.append(fidelity_ideal(input_ensemble,r,n*noise_mat))
plt.plot(noise_list,fid_noisy)
