...
def teleportation_protocol(s_list,theta_list,idx_list,wormhole,n_one_side,H_coupling,coupling):
    if wormhole == False:
        N = 2*n_one_side
        k = 5
        m_squared = 13
    ...
    for i in range(len(idx_list)):
        ...
        Gamma_insert = insert_unentangled_mode(Gamma_insert, idx_list[i], Gamma_rot_squeezed)
...
def fidelity_vs_site(
    ...
    Vins = [[] for _ in range(len(idx_list))]
 
    Vouts = [[] for _ in range(2*N)]


    for x in range(len(input_ensemble[0][0])):
        # Run your usual protocol (NO observer) to get global Gamma_final
        s_list = []
        theta_list = []
        for i in range(len(idx_list)):
            s_list.append(input_ensemble[i][0][x][0])
            theta_list.append(input_ensemble[i][0][x][1])
        Gamma_final= teleportation_protocol(
                s_list=s_list, theta_list=theta_list, idx_list = idx_list, wormhole=wormhole,n_one_side=N,H_coupling=H_coupling,coupling = True
            )        

        for i in range(len(idx_list)):    
            Vins[i].append(make_input_covariance(s_list[i],theta_list[i]))
    ...
    fid_symp = [[] for _ in range(len(idx_list))]
    fid_flip = [[] for _ in range(len(idx_list))]

    for i in range(len(idx_list)):
        for j in range(2*N):
            X, Y = fit_gaussian_channel(Vins[i], Vouts[j])
            ...
            fid_symp[i].append(Fs)
            fid_flip[i].append(Ff)
...
input_ensemble = [[] for _ in range(len(idx_list))]
for i in range(len(idx_list)):
    #a = 1+1/idx_list[i]
    #if a >=.9:
        #a = a/2
    a = 1
    Ss = np.linspace(-1*a, 1*a, 4)
    Thetas = np.linspace(0, 2*np.pi*a, 3, endpoint=False)
    input_ensemble[i].append([(s, th) for s in Ss for th in Thetas]) # 120 points 
...
Fs,Ff= fidelity_vs_site(idx_list,input_ensemble,H_coupling_OG,N=N,wormhole=False) 


insert_idx = idx_list[0]
#plt.plot(sites,Fs,label="symplectic")
plt.rc('font', size=14)
plt.plot(sites,Ff[0],color="k",linewidth=2)
...
insert_idx = idx_list[1]
#plt.plot(sites,Fs,label="symplectic")
plt.rc('font', size=14)
plt.plot(sites,Ff[1],color="k",linewidth=2)
plt.axvline(insert_idx,color="blue",linestyle="dashed",linewidth=2,label="insert site")
plt.axvline(insert_idx + N,color="red",linestyle="dashed",linewidth=2,label="teleport site")
plt.xlabel("site")
plt.ylabel("fidelity")
